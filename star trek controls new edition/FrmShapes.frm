VERSION 5.00
Begin VB.Form FrmShapes 
   BackColor       =   &H00000000&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Form1"
   ClientHeight    =   5265
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10500
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5265
   ScaleWidth      =   10500
   StartUpPosition =   2  'CenterScreen
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape7 
      Height          =   2235
      Left            =   4020
      TabIndex        =   9
      Top             =   360
      Width           =   2055
      _ExtentX        =   3625
      _ExtentY        =   3942
      Shape           =   2
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape2 
      Height          =   1095
      Left            =   300
      TabIndex        =   0
      Top             =   240
      Width           =   735
      _ExtentX        =   1296
      _ExtentY        =   1931
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   33023
      FixedWidth      =   30
      FixedHeight     =   30
      Shape           =   3
      VerticalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape3 
      Height          =   1755
      Left            =   360
      TabIndex        =   1
      Top             =   1260
      Width           =   435
      _ExtentX        =   873
      _ExtentY        =   3096
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   29
      FixedHeight     =   30
      Shape           =   4
      HorizontalOrientation=   1
      LineWidth       =   3
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape1 
      Height          =   1095
      Left            =   360
      TabIndex        =   2
      Top             =   3000
      Width           =   735
      _ExtentX        =   1296
      _ExtentY        =   1931
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   33023
      FixedWidth      =   30
      FixedHeight     =   30
      Shape           =   3
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape4 
      Height          =   1095
      Left            =   2880
      TabIndex        =   3
      Top             =   240
      Width           =   735
      _ExtentX        =   1296
      _ExtentY        =   1931
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   33023
      FixedWidth      =   30
      FixedHeight     =   30
      Shape           =   3
      VerticalOrientation=   1
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape5 
      Height          =   1755
      Left            =   3120
      TabIndex        =   4
      Top             =   1260
      Width           =   435
      _ExtentX        =   873
      _ExtentY        =   3096
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   29
      FixedHeight     =   30
      Shape           =   4
      LineWidth       =   3
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape6 
      Height          =   1095
      Left            =   2820
      TabIndex        =   5
      Top             =   3000
      Width           =   735
      _ExtentX        =   1296
      _ExtentY        =   1931
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   33023
      FixedWidth      =   30
      FixedHeight     =   30
      Shape           =   3
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekButton StarTrekButton11 
      Height          =   435
      Left            =   900
      TabIndex        =   6
      Top             =   1380
      Width           =   2115
      _ExtentX        =   3731
      _ExtentY        =   767
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   238
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      BackColor       =   16384
      TransparentColor=   16384
      BackColorMiddle1=   16733618
      BackColorMiddle2=   16744140
      BackColorMiddle1Hover=   4454843
      BackColorMiddle2Hover=   12648384
      BackColorMiddle1Down=   16733618
      BackColorMiddle2down=   16733618
      BackColorMiddle1Disabled=   8421504
      BackColorMiddle2Disabled=   8421504
      BackColorLeft   =   16733618
      BackColorLeftHover=   4454843
      BackColorLeftDown=   16733618
      BackColorLeftDisabled=   8421504
      BackColorRight  =   16744140
      BackColorRightHover=   12648384
      BackColorRightDown=   16733618
      BackColorRightDisabled=   8421504
      ForeColorDisabled=   4210752
      Style           =   6
      GradientAngle   =   0
      SpacingLeftHover=   1
      SpacingLeftDown =   0
      SpacingRight    =   0
      SpacingRightHover=   0
      SpacingRightDown=   0
      SpacingRightDisabled=   0
      AutoFillSides   =   -1  'True
   End
   Begin StarTrekControlsNE.StarTrekButton StarTrekButton2 
      Height          =   435
      Left            =   900
      TabIndex        =   7
      Top             =   1920
      Width           =   2115
      _ExtentX        =   3731
      _ExtentY        =   767
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   238
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      BackColor       =   16384
      TransparentColor=   16384
      BackColorMiddle1=   16733618
      BackColorMiddle2=   16744140
      BackColorMiddle1Hover=   4454843
      BackColorMiddle2Hover=   12648384
      BackColorMiddle1Down=   16733618
      BackColorMiddle2down=   16733618
      BackColorMiddle1Disabled=   8421504
      BackColorMiddle2Disabled=   8421504
      BackColorLeft   =   16733618
      BackColorLeftHover=   4454843
      BackColorLeftDown=   16733618
      BackColorLeftDisabled=   8421504
      BackColorRight  =   16744140
      BackColorRightHover=   12648384
      BackColorRightDown=   16733618
      BackColorRightDisabled=   8421504
      ForeColorDisabled=   4210752
      Style           =   6
      GradientAngle   =   0
      SpacingLeftHover=   1
      SpacingLeftDown =   0
      SpacingRight    =   0
      SpacingRightHover=   0
      SpacingRightDown=   0
      SpacingRightDisabled=   0
      AutoFillSides   =   -1  'True
   End
   Begin StarTrekControlsNE.StarTrekButton StarTrekButton3 
      Height          =   435
      Left            =   900
      TabIndex        =   8
      Top             =   2460
      Width           =   2115
      _ExtentX        =   3731
      _ExtentY        =   767
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "Verdana"
         Size            =   9.75
         Charset         =   238
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      BackColor       =   16384
      TransparentColor=   16384
      BackColorMiddle1=   16733618
      BackColorMiddle2=   16744140
      BackColorMiddle1Hover=   4454843
      BackColorMiddle2Hover=   12648384
      BackColorMiddle1Down=   16733618
      BackColorMiddle2down=   16733618
      BackColorMiddle1Disabled=   8421504
      BackColorMiddle2Disabled=   8421504
      BackColorLeft   =   16733618
      BackColorLeftHover=   4454843
      BackColorLeftDown=   16733618
      BackColorLeftDisabled=   8421504
      BackColorRight  =   16744140
      BackColorRightHover=   12648384
      BackColorRightDown=   16733618
      BackColorRightDisabled=   8421504
      ForeColorDisabled=   4210752
      Style           =   6
      GradientAngle   =   0
      SpacingLeftHover=   1
      SpacingLeftDown =   0
      SpacingRight    =   0
      SpacingRightHover=   0
      SpacingRightDown=   0
      SpacingRightDisabled=   0
      AutoFillSides   =   -1  'True
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape8 
      Height          =   2235
      Left            =   6180
      TabIndex        =   10
      Top             =   360
      Width           =   2055
      _ExtentX        =   3625
      _ExtentY        =   3942
      Shape           =   2
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape9 
      Height          =   2235
      Left            =   4020
      TabIndex        =   11
      Top             =   2700
      Width           =   2055
      _ExtentX        =   3625
      _ExtentY        =   3942
      Shape           =   2
      VerticalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape10 
      Height          =   2235
      Left            =   6180
      TabIndex        =   12
      Top             =   2700
      Width           =   2055
      _ExtentX        =   3625
      _ExtentY        =   3942
      Shape           =   2
      VerticalOrientation=   1
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape11 
      Height          =   2235
      Left            =   8520
      TabIndex        =   13
      Top             =   2700
      Width           =   675
      _ExtentX        =   1191
      _ExtentY        =   3942
      VerticalOrientation=   1
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape12 
      Height          =   2235
      Left            =   8520
      TabIndex        =   14
      Top             =   360
      Width           =   675
      _ExtentX        =   1191
      _ExtentY        =   3942
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape14 
      Height          =   555
      Left            =   9420
      TabIndex        =   15
      Top             =   420
      Width           =   675
      _ExtentX        =   1191
      _ExtentY        =   979
      Shape           =   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape13 
      Height          =   555
      Left            =   9420
      TabIndex        =   16
      Top             =   1080
      Width           =   675
      _ExtentX        =   1191
      _ExtentY        =   979
      Shape           =   1
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape15 
      Height          =   615
      Left            =   540
      TabIndex        =   17
      Top             =   3780
      Width           =   555
      _ExtentX        =   979
      _ExtentY        =   1085
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
      LineAngle       =   25
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape16 
      Height          =   255
      Left            =   540
      TabIndex        =   18
      Top             =   4140
      Width           =   2895
      _ExtentX        =   5106
      _ExtentY        =   450
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape17 
      Height          =   615
      Left            =   2880
      TabIndex        =   19
      Top             =   3780
      Width           =   555
      _ExtentX        =   979
      _ExtentY        =   1085
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
      LineAngle       =   155
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape18 
      Height          =   255
      Left            =   1020
      TabIndex        =   20
      Top             =   3900
      Width           =   195
      _ExtentX        =   344
      _ExtentY        =   450
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
      LineAngle       =   90
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape19 
      Height          =   255
      Left            =   2760
      TabIndex        =   21
      Top             =   3900
      Width           =   195
      _ExtentX        =   344
      _ExtentY        =   450
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
      LineAngle       =   90
   End
   Begin StarTrekControlsNE.StarTrekShape StarTrekShape20 
      Height          =   195
      Left            =   1140
      TabIndex        =   22
      Top             =   4020
      Width           =   1755
      _ExtentX        =   3096
      _ExtentY        =   344
      BackColor       =   8388736
      TransparentColor=   8388736
      ShapeColor      =   8438015
      FixedWidth      =   37
      FixedHeight     =   30
      Shape           =   5
      HorizontalOrientation=   1
   End
End
Attribute VB_Name = "FrmShapes"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'============== properties: ========================================================
'
'---- shape ------------------
'       >FixedHeight: use this property for federation and romulan styles
'                     allow you to define height of one part of control
'                     and another one will be used from control height
'       >FixedWidth: same as FixedHeight
'       >HotizontalOrientation: left or right
'       >Shape: it's something like style
'       >lineAngle: use this property for customLine shape/style
'       >lineWidth: use this property for Romulan and customLine shapes
'                   alloe you to define line width
'       >verticalOrientation: top or bottom
'

